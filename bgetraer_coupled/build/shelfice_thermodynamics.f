...
C                     pressure gradient in Y (no Units)
     ...
     &                dU_psFacX, dV_psFacY
      Real*8  dU_psFacX(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
      Real*8  dV_psFacY(1-OLx:sNx+OLx,1-OLy:sNy+OLy,Nr,nSx,nSy)
CBOP
C     !ROUTINE: FFIELDS.h
C     !INTERFACE:
C     include "FFIELDS.h"
...
CEOP
     ...
     &     shelfice_extern_forcing_cycle
      ...
      Real*8 SHELFICEremeshFrequency
      Real*8 SHELFICEsplitThreshold
      Real*8 SHELFICEmergeThreshold
      Real*8 SHELFICEptracConc
      Real*8 shelfice_forcing_period		  
      Real*8 shelfice_extern_forcing_cycle
     ...
     &     shelficeSubglFlux0, shelficeSubglFlux1
      ...
      Real*8 shelficeSubglFlux     (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 shelficeSubglFlux0    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 shelficeSubglFlux1    (1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
     ...
     &   shelfIceMassDynTendency(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      ...
      Real*8 shelficeDragU(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
      Real*8 shelficeDragV(1-OLx:sNx+OLx,1-OLy:sNy+OLy,nSx,nSy)
     ...
     &     SHELFICEMassStepping, shelfice_conserve_ssh,
...
C     drKp1, recip_drLoc
      ...
      Real*8 uLoc(1:sNx+1,1:sNy+1)
      Real*8 vLoc(1:sNx+1,1:sNy+1)
      Real*8 velSq(1:sNx,1:sNy)
      Real*8 thetaFreeze, saltFreeze, recip_Cp
      Real*8 freshWaterFlux
      Real*8 a0, a1, a2, b, c0
      Real*8 eps1, eps2, eps3, eps3a, eps4, eps6, eps7, eps8
      Real*8 cFac, rFac, dFac, fwflxFac, rFWinBL
      Real*8 aqe, bqe, cqe, discrim, recip_aqe
      Real*8 drKp1, recip_drLoc
      Real*8 recip_latentHeat
      Real*8 tmpFac

      Real*8 shiPr, shiSc, shiLo, recip_shiKarman, shiTwoThirds
      Real*8 gammaTmoleT, gammaTmoleS, gammaTurb, gammaTurbConst
      Real*8 ustar, ustarSq, etastar
      Real*8 u_tmp, v_tmp
...
cph      shiSc = (viscArNr(1)/diffKrNrS(1))**shiTwoThirds
       gammaTmoleT = 12.5D0 * shiPr - 6.D0
       gammaTmoleS = 12.5D0 * shiSc - 6.D0
C     instead of etastar = sqrt(1+zetaN*ustar./(f*Lo*Rc))
       etastar = 1.D0
       gammaTurbConst  = 1.D0 / (2.D0 * shiZetaN*etastar)
     &      - recip_shiKarman
      ENDIF
...
C     with "real fresh water flux" (affecting ETAN), there is more to modify
      ...
      rFWinBL = 0.D0
      IF ( SHI_withBL_realFWflux ) rFWinBL = 1.D0
...
c         pLoc(I,J) = shelficeMass(I,J,bi,bj)*gravity*1.D-4
...
C-    Original averaging expression for uStar:
     ...
     &         + uVel(I+1,J,K,bi,bj) * hFacW(I+1,J,K,bi,bj) )
           vLoc(I,J) = recip_hFacC(I,J,K,bi,bj) * halfRL *
     &         ( vVel(I,J,  K,bi,bj) * hFacS(I,J,  K,bi,bj)
     &         + vVel(I,J+1,K,bi,bj) * hFacS(I,J+1,K,bi,bj) )
...
C-    New (more accurate) averaging expression for uStar:
         DO J = 1, sNy
          DO I = 1, sNx
           ...
           velSq(I,J) = 0.
           K = MAX(1,kTopC(I,J,bi,bj))
           tmpFac = hFacW(I,  J,K,bi,bj) + hFacW(I+1,J,K,bi,bj)
           IF ( tmpFac.GT.0.D0 )
     &     velSq(I,J) = (
     &     uVel( I, J,K,bi,bj)*uVel( I, J,K,bi,bj)*hFacW( I, J,K,bi,bj)
     &   + uVel(I+1,J,K,bi,bj)*uVel(I+1,J,K,bi,bj)*hFacW(I+1,J,K,bi,bj)
     &                  )/tmpFac
           tmpFac = hFacS(I,J,  K,bi,bj) + hFacS(I,J+1,K,bi,bj)
           IF ( tmpFac.GT.0.D0 )
     &     velSq(I,J) = velSq(I,J) + (
     &     vVel(I, J, K,bi,bj)*vVel(I, J, K,bi,bj)*hFacS(I, J, K,bi,bj)
     &   + vVel(I,J+1,K,bi,bj)*vVel(I,J+1,K,bi,bj)*hFacS(I,J+1,K,bi,bj)
     &                               )/tmpFac
          ENDDO
         ENDDO
        ENDIF
...
C--   lower cell may not be as thick as required
     ...
     &           * recip_drLoc
            uLoc(I,J) = ( uLoc(I,J) * drF(K)*hFacC(I,J,K,bi,bj)
     &           + drKp1 * recip_hFacC(I,J,Kp1,bi,bj) * halfRL *
     &           ( uVel(I,  J,Kp1,bi,bj) * hFacW(I,  J,Kp1,bi,bj)
     &           + uVel(I+1,J,Kp1,bi,bj) * hFacW(I+1,J,Kp1,bi,bj) )
     &           ) * recip_drLoc
            vLoc(I,J) = ( vLoc(I,J) * drF(K)*hFacC(I,J,K,bi,bj)
     &           + drKp1 * recip_hFacC(I,J,Kp1,bi,bj) * halfRL *
     &           ( vVel(I,J,  Kp1,bi,bj) * hFacS(I,J,  Kp1,bi,bj)
     &           + vVel(I,J+1,Kp1,bi,bj) * hFacS(I,J+1,Kp1,bi,bj) )
     &           ) * recip_drLoc
            velSq(I,J) = uLoc(I,J)*uLoc(I,J)+vLoc(I,J)*vLoc(I,J)
           ENDIF
          ENDDO
...
C    Average at u- and v- points over deltaR. use later to override uLoc/vLoc
         DO J = 1, sNy+1
          DO I = 1, sNx+1
           ...
           IF (K.LT.Nr) THEN
            Kp1 = K+1
            drKp1 = drF(K)*( 1.D0 - hFacW(I,J,K,bi,bj) )
            drKp1 = MIN( drKp1, drF(Kp1) * hFacW(I,J,Kp1,bi,bj) )
            drKp1 = MAX( drKp1, 0.D0 )
            recip_drLoc = 1.D0
     &                  / ( drF(K)*hFacW(I,J,K,bi,bj) + drKp1 )
            uLoc(I,J) = ( drF(K)*hFacW(I,J,K,bi,bj)*uVel(I,J,K,bi,bj)
     &                  + drKp1*uVel(I,J,Kp1,bi,bj)
     &                  )*recip_drLoc
c           u_topDr(I,J) =
c    &           ( uVel(I,J,K,bi,bj)*drF(K)*hFacW(I,J,K,bi,bj)
...
c    &           )*recip_drLoc
           ...
           IF (K.LT.Nr) THEN
            ...
            drKp1 = MIN( drKp1, drF(Kp1) * hFacS(I,J,Kp1,bi,bj) )
            drKp1 = MAX( drKp1, 0.D0 )
            recip_drLoc = 1.D0
     &                  / ( drF(K)*hFacS(I,J,K,bi,bj) + drKp1 )
            vLoc(I,J) = ( drF(K)*hFacS(I,J,K,bi,bj)*vVel(I,J,K,bi,bj)
     &                  + drKp1*vVel(I,J,Kp1,bi,bj)
     &                  )*recip_drLoc
c           v_topDr(I,J) =
c    &           ( vVel(I,J,K,bi,bj)*drF(K)*hFacS(I,J,K,bi,bj)
...
C     instead of etastar = sqrt(1+zetaN*ustar./(f*Lo*Rc))
C           etastar = 1.D0
C           gammaTurbConst  = 1.D0 / (2.D0 * shiZetaN*etastar)
C    &           - recip_shiKarman
            IF ( fCori(I,J,bi,bj) .NE. 0.D0 ) THEN
             gammaTurb = LOG( ustarSq * shiZetaN * etastar**2
     &            / ABS(fCori(I,J,bi,bj) * 5.0D0 * shiKinVisc))
     &            * recip_shiKarman
     &            + gammaTurbConst
