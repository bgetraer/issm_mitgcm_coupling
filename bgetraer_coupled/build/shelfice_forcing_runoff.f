...
!!! ADD CLAUSE FOR klowc==1         
     ...
     &          /( drF(kLev)*hFacC(I,J,kLev,bi,bj))
          gT_arr(i,j) = gT_arr(i,j) + gTloc
         ELSEIF ( kLev .GT. 1 .AND. kLev .EQ. kLowC(I,J,bi,bj) ) THEN
          km1 = MAX(kLev-1,1)
          drLoc = min_thick_cell - drF(kLev)*hFacC(I,J,kLev,bi,bj) 
          drLoc = MIN( drLoc, drF(Km1) * hFacC(I,J,Km1,bi,bj) )
...
!!! ADD CLAUSE FOR klowc==1         
         ...
         IF (klev.eq.1 .and. kLev .EQ. kLowC(I,J,bi,bj)) THEN
          gSloc = rflux * (0.D0  - salt(i,j,klev,bi,bj))
     &          /( drF(kLev)*hFacC(I,J,kLev,bi,bj))
     ...
     &         /( drF(kLev)*hFacC(I,J,kLev,bi,bj)+drLoc )
         ...
         ELSEIF ( kLev .LT. Nr .AND. kLev+1 .EQ. kLowC(I,J,bi,bj) ) THEN
          ...
          drLoc = MIN( drLoc, drF(kLev) * hFacC(I,J,kLev,bi,bj) )
          drLoc = MAX( drLoc, 0.D0)
          gSloc = rflux * (0.D0 - salt(i,j,klev+1,bi,bj))
     &         /( drF(kp1)*hFacC(I,J,kp1,bi,bj)+drLoc )
          gS_arr(i,j) = gS_arr(i,j) + gSloc
     &         * drLoc*recip_drF(kLev)* recip_hFacC(i,j,kLev,bi,bj)
         ENDIF
...
C     myIter    :: Current iteration number
C     myThid    :: my Thread Id number
      Real*8     gPtr_arr(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     Ptr_arr(1-OLx:sNx+OLx,1-OLy:sNy+OLy)
      Real*8     Ptr_conc
      INTEGER iMin, iMax, jMin, jMax
      INTEGER kLev, bi, bj
      Real*8     myTime
...
!!! ADD CLAUSE FOR klowc==1         
         ...
         IF (klev.eq.1 .and. kLev .EQ. kLowC(I,J,bi,bj)) THEN
          gPtrloc = rflux * (ptr_Conc - ptr_arr(i,j))
     &          /( drF(kLev)*hFacC(I,J,kLev,bi,bj))
