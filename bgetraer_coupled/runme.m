...
if perform(org,'MeshParam'),% {{{
	...
	%Temperature
	md.initialization.temperature=(273-15)*ones(md.mesh.numberofvertices,1); % deg K
	...
	q=80E9;											% integrated flux across inflow boundary, per Nakayama2021 (m^3/yr)
	b=LxOC/2;										% location of apex of parabola (m)
	a=q/(H0*4/3*b^3);								% scaling to enforce the integrated flux
	V0=a.*(-(md.mesh.x(pos)-b).^2 + b^2);	% the parabolic equation for velocity on the boundary (m/yr)
	md.stressbalance.spcvy(pos)=V0;
	...
	% define initial ice shelf mass, open z cells, and free surface; write to input files {{{
	   ...
	   % }}}
		% read ice shelf thickness from ISSM {{{
	      md = loadmodel([fbase 'Models/PigLike.SteadystateNoSlip']);
	   ...
	   % }}}
		% calculate depth of the ice draft and free surface {{{
	      ...
	      %BENJY: why isnt this the same as data.shelfice?
	      mergethreshold=0.25; % dimensionless merge threshold
